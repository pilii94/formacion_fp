...
def get_genres(movies):
	genres_set = set()
	for genre in list(movies['genres']):
		for g in genre.split('|'):
			genres_set.add(g)
	return list(genres_set)

def get_genre_ratings(ratings, genres, column_names):
    genre_ratings = pd.DataFrame()
    for genre in genres:        
        genre_movies = ratings[ratings['genres'].str.contains(genre) ]
        avg_genre_votes_per_user = ratings[ratings['movieId'].isin(genre_movies['movieId'])].loc[:, ['userId', 'rating']].groupby(['userId'])['rating'].mean().round(2)
        avg_genre_votes_per_user.fillna(avg_genre_votes_per_user.mean())
...
def draw_clusters(biased_dataset, predictions, labels, cmap='viridis'):
    label1 = labels[0]
    label2 = labels[1]
    ...
    plt.xlim(0, 5)
    plt.ylim(0, 5)
    ax.set_xlabel('Avg %s rating' % label1, fontsize=18)
    ax.set_ylabel('Avg %s rating' % label2, fontsize=18)
    

    clustered = pd.concat([biased_dataset.reset_index(), pd.DataFrame({'group':predictions})], axis=1)
    plt.scatter(clustered['avg_'+label1+'_rating'], clustered['avg_'+label2+'_rating'], c=clustered['group'], s=20, cmap=cmap)
...
def draw_movie_clusters(clustered, max_users, max_movies):
    ...
    for cluster_id in clustered.group.unique():
        ...
        if len(d) > 9:
            ...
            print('# Usuarios en el cluster: {}.'.format(n_users_in_cluster), '# of users in plot: {}'.format(n_users_in_plot))
